...
public final class NetworkProposalLifecycleBehaviorTest {
    ...
    void task7_proposalLifecycleBehavior_executionResumeAndResponseFlowEndsInDone() {
        ...
        assertTrue("task7/proposal-lifecycle-behavior/start-executing",
                MineAgentNetwork.SESSIONS.tryStartExecuting(sessionId, first.id()));

        MineAgentNetwork.SESSIONS.clearProposalPreserveState(sessionId);
    ...
    void task7_approvalDecisionBehavior_denyClearsProposalAndRecordsDecision() {
        ...
        MineAgentNetwork.SESSIONS.setProposal(sessionId, proposal, binding("west"));
        MineAgentNetwork.SESSIONS.setActive(PLAYER_ID, sessionId);
    ...
    void task7_deletedSession_asyncAppendPath_doesNotRecreateSession() {
        ...
        MineAgentNetwork.appendMessageAndBroadcast(sessionId,
    ...
    private static UUID newSession() {
        return MineAgentNetwork.SESSIONS.create(PLAYER_ID, PLAYER_NAME).metadata().sessionId();
    }
    ...
    private static void invokeApplyAgentError(UUID sessionId, String message) {
        try {
            Method method = MineAgentNetwork.class.getDeclaredMethod("applyAgentError", UUID.class, String.class);
            method.setAccessible(true);
    ...
    private static SessionSnapshot invokeSnapshotRoundTrip(SessionSnapshot snapshot) {
        try {
            Method write = MineAgentNetwork.class.getDeclaredMethod("writeSnapshot", FriendlyByteBuf.class, SessionSnapshot.class);
            Method read = MineAgentNetwork.class.getDeclaredMethod("readSnapshot", FriendlyByteBuf.class);
    ...
    private static void invokeSubscribeViewer(UUID viewerId, UUID sessionId) {
        try {
            Method method = MineAgentNetwork.class.getDeclaredMethod("subscribeViewer", UUID.class, UUID.class);
            method.setAccessible(true);
    ...
    private static void invokeUnsubscribeViewer(UUID viewerId) {
        try {
            Method method = MineAgentNetwork.class.getDeclaredMethod("unsubscribeViewer", UUID.class);
            method.setAccessible(true);
