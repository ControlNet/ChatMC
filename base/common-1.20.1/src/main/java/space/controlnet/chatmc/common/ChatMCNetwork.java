...
import space.controlnet.chatmc.core.audit.AuditOutcome;
...
import space.controlnet.chatmc.core.session.ServerSessionManager;
...
import space.controlnet.chatmc.core.util.ItemTagParser;
import space.controlnet.chatmc.core.util.LocaleResolver;
import space.controlnet.chatmc.core.net.c2s.C2SApprovalDecisionPacket;
import space.controlnet.chatmc.core.net.c2s.C2SCreateSessionPacket;
import space.controlnet.chatmc.core.net.c2s.C2SDeleteSessionPacket;
import space.controlnet.chatmc.core.net.c2s.C2SOpenSessionPacket;
import space.controlnet.chatmc.core.net.c2s.C2SRequestSessionListPacket;
import space.controlnet.chatmc.core.net.c2s.C2SSendChatPacket;
import space.controlnet.chatmc.core.net.c2s.C2SUpdateSessionPacket;
import space.controlnet.chatmc.core.net.s2c.S2CSessionListPacket;
import space.controlnet.chatmc.core.net.s2c.S2CSessionSnapshotPacket;
...
public final class ChatMCNetwork {
    ...
    private static final ExecutorService LLM_EXECUTOR = Executors.newFixedThreadPool(2, r -> {
        Thread t = new Thread(r, "chatmc-llm");
        t.setDaemon(true);
        return t;
    });
    ...
    private static final java.util.concurrent.atomic.AtomicReference<ChatMCSessionsSavedData> SAVED_SESSIONS = new java.util.concurrent.atomic.AtomicReference<>();
    ...
    public static void setServer(MinecraftServer server) {
        if (server == null) {
            ChatMCSessionsSavedData saved = SAVED_SESSIONS.getAndSet(null);
            if (saved != null) {
        ...
        ChatMCSessionsSavedData saved = ChatMCSessionsSavedData.get(server);
    ...
    private static Optional<String> findInvalidItemTag(String text) {
        return ItemTagParser.findInvalidItemTag(text, ChatMCNetwork::isValidItemId);
    }
    ...
    private static void persistSessions() {
        ChatMCSessionsSavedData saved = SAVED_SESSIONS.get();
        if (saved == null) {
            return;
        }
        saved.setData(SESSIONS.exportForSave());
    ...
    private static void handleChatPacket(ServerPlayer player, String text, String clientLocale, String aiLocaleOverride) {
        ...
        if (invalidItemId.isPresent()) {
            ChatMC.LOGGER.warn("Player {} sent message with invalid item ID: {}", player.getGameProfile().getName(), invalidItemId.get());
    ...
    private static Optional<TerminalBinding> captureTerminalBinding(ServerPlayer player) {
        if (!(player.containerMenu instanceof space.controlnet.chatmc.common.menu.AiTerminalMenu menu)) {
            return Optional.empty();
        }
    ...
    private static final class AgentInvoker {
        ...
        space.controlnet.chatmc.core.agent.AgentLoopResult runLoop(ServerPlayer player, UUID sessionId, TerminalBinding binding, String effectiveLocale) {
            Object instance = ensureRunner();
            if (instance == null) {
                return space.controlnet.chatmc.core.agent.AgentLoopResult.withError("Agent runner not available", 0);
            }
            try {
                ...
                return result instanceof space.controlnet.chatmc.core.agent.AgentLoopResult loopResult
                        ? loopResult
                        : space.controlnet.chatmc.core.agent.AgentLoopResult.withError("Agent returned invalid result", 0);
            } catch (Throwable t) {
                ChatMC.LOGGER.error("Agent invocation failed, disabling agent", t);
                runner = null;
                return space.controlnet.chatmc.core.agent.AgentLoopResult.withError("Agent invocation failed: " + t.getMessage(), 0);
            }
        }
        ...
        void setRateLimitCooldown(long cooldownMs) {
            ...
            try {
                ...
                ChatMC.LOGGER.warn("Failed to set rate limit cooldown", t);
            }
        }
        ...
        void setTimeoutMs(long timeoutMs) {
            ...
            try {
                ...
                ChatMC.LOGGER.warn("Failed to set timeout", t);
            }
        }
        ...
        void setMaxToolCalls(int maxToolCalls) {
            ...
            try {
                ...
                ChatMC.LOGGER.warn("Failed to set max tool calls", t);
            }
        }
        ...
        void setLogResponses(boolean logResponses) {
            ...
            try {
                ...
                ChatMC.LOGGER.warn("Failed to set log responses flag", t);
            }
        }
        ...
        void setMaxRetries(int maxRetries) {
            ...
            try {
                ...
                ChatMC.LOGGER.warn("Failed to set max retries", t);
            }
        }
        ...
        private Object ensureRunner() {
            ...
            initAttempted = true;
            try {
                Class<?> clazz = Class.forName("space.controlnet.chatmc.common.agent.AgentRunner");
                runner = clazz.getConstructor().newInstance();
                return runner;
            } catch (Throwable t) {
                ChatMC.LOGGER.warn("Agent runtime unavailable, continuing without LLM support", t);
                runner = null;
                return null;
            }
        }
    ...
    private static void handleApprovalDecision(ServerPlayer player, C2SApprovalDecisionPacket packet) {
        ...
        Optional<space.controlnet.chatmc.core.terminal.TerminalContext> context = binding.isPresent()
