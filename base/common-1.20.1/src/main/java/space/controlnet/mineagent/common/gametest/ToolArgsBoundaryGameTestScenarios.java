...
public final class ToolArgsBoundaryGameTestScenarios {
    private static final int MAX_TOOL_ARGS_JSON_LENGTH = 65_536;

    private ToolArgsBoundaryGameTestScenarios() {
    }
    ...
    private static void assertAcceptsAcrossParseNetworkPersistence(
        ...
        AgentGameTestSupport.requireEquals(assertionPrefix + "/expected-length", expectedLength, argsJson.length());
        ...
        AgentGameTestSupport.requireEquals(assertionPrefix + "/network-roundtrip/args-equals", argsJson, networkArgs);
        ...
        AgentGameTestSupport.requireEquals(assertionPrefix + "/persist-roundtrip/args-equals", argsJson, persistedArgs);
    }
    ...
    private static FriendlyByteBuf invokeNetworkWriteSnapshot(SessionSnapshot snapshot) {
        try {
            ...
            throw new AssertionError("task10/reflection/invoke-network-write-snapshot", AgentGameTestSupport.rootCause(exception));
        }
    }
    ...
    private static SessionSnapshot invokeNetworkReadSnapshot(FriendlyByteBuf buf) {
        try {
            ...
            throw new AssertionError("task10/reflection/invoke-network-read-snapshot", AgentGameTestSupport.rootCause(exception));
        }
    }
    ...
    private static CompoundTag invokeWriteSession(SessionSnapshot snapshot) {
        try {
            ...
            throw new AssertionError("task10/reflection/invoke-persist-write-session", AgentGameTestSupport.rootCause(exception));
        }
    }
    ...
    private static SessionSnapshot invokeReadSession(CompoundTag root) {
        try {
            ...
            throw new AssertionError("task10/reflection/invoke-persist-read-session", AgentGameTestSupport.rootCause(exception));
        }
    }
    ...
    private static void invokeParseBoundaryValidate(String toolName, String argsJson) {
        try {
            ...
            throw new AssertionError("task10/reflection/invoke-parse-boundary-validate", AgentGameTestSupport.rootCause(exception));
        }
    }
    ...
    private static String buildPayloadWithSeedAtLength(String assertionName, String seed, int targetLength) {
        AgentGameTestSupport.requireTrue(assertionName + "/seed-not-empty", seed != null && !seed.isEmpty());
        AgentGameTestSupport.requireTrue(assertionName + "/seed-fits-target", seed.length() <= targetLength);
        ...
        AgentGameTestSupport.requireEquals(assertionName + "/target-length", targetLength, payload.length());
        return payload;
    }
