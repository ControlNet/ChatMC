...
import space.controlnet.mineagent.core.session.DecisionLogEntry;
...
import space.controlnet.mineagent.core.net.c2s.C2SCreateSessionPacket;
import space.controlnet.mineagent.core.net.c2s.C2SDeleteSessionPacket;
import space.controlnet.mineagent.core.net.c2s.C2SOpenSessionPacket;
import space.controlnet.mineagent.core.net.c2s.C2SRequestSessionListPacket;
import space.controlnet.mineagent.core.net.c2s.C2SSendChatPacket;
import space.controlnet.mineagent.core.net.c2s.C2SUpdateSessionPacket;
import space.controlnet.mineagent.core.net.s2c.S2CSessionListPacket;
import space.controlnet.mineagent.core.net.s2c.S2CSessionSnapshotPacket;
...
public final class MineAgentNetwork {
    private static final int MAX_MESSAGE_LENGTH = Integer.getInteger("mineagent.maxMessageLength", 65536);
    ...
    private static final ExecutorService LLM_EXECUTOR = Executors.newFixedThreadPool(2, r -> {
        Thread t = new Thread(r, "mineagent-llm");
        t.setDaemon(true);
        return t;
    });
    ...
    private static final java.util.concurrent.atomic.AtomicReference<MineAgentSessionsSavedData> SAVED_SESSIONS = new java.util.concurrent.atomic.AtomicReference<>();
    ...
    public static void setServer(MinecraftServer server) {
        if (server == null) {
            MineAgentSessionsSavedData saved = SAVED_SESSIONS.getAndSet(null);
            if (saved != null) {
        ...
        MineAgentSessionsSavedData saved = MineAgentSessionsSavedData.get(server);
    ...
    private static Optional<String> findInvalidItemTag(String text) {
        return ItemTagParser.findInvalidItemTag(text, MineAgentNetwork::isValidItemId);
    }
    ...
    private static void persistSessions() {
        MineAgentSessionsSavedData saved = SAVED_SESSIONS.get();
        if (saved == null) {
            return;
        }
        saved.setData(SESSIONS.exportForSave());
    ...
    private static void handleChatPacket(ServerPlayer player, String text, String clientLocale, String aiLocaleOverride) {
        ...
        if (invalidItemId.isPresent()) {
            MineAgent.LOGGER.warn("Player {} sent message with invalid item ID: {}", player.getGameProfile().getName(), invalidItemId.get());
    ...
    private static Optional<TerminalBinding> captureTerminalBinding(ServerPlayer player) {
        if (!(player.containerMenu instanceof space.controlnet.mineagent.common.menu.AiTerminalMenu menu)) {
            return Optional.empty();
        }
    ...
    private static CompletableFuture<space.controlnet.mineagent.core.agent.AgentLoopResult> runAgentLoopAsync(
    ...
    private static void handleAgentLoopResult(ServerPlayer player,
            space.controlnet.mineagent.core.agent.AgentLoopResult result, UUID sessionId, TerminalBinding binding, String effectiveLocale) {
        if (SESSIONS.get(sessionId).isEmpty()) {
    ...
    private static void sendSessionList(ServerPlayer player, SessionListScope scope) {
        ...
        List<SessionSummary> sessions = SESSIONS.listAll().stream()
                ...
                .map(MineAgentNetwork::toSummary)
    ...
    private static final class AgentInvoker {
        ...
        space.controlnet.mineagent.core.agent.AgentLoopResult runLoop(ServerPlayer player, UUID sessionId, TerminalBinding binding, String effectiveLocale) {
            Object instance = ensureRunner();
            if (instance == null) {
                return space.controlnet.mineagent.core.agent.AgentLoopResult.withError("Agent runner not available", 0);
            }
            try {
                ...
                return result instanceof space.controlnet.mineagent.core.agent.AgentLoopResult loopResult
                        ? loopResult
                        : space.controlnet.mineagent.core.agent.AgentLoopResult.withError("Agent returned invalid result", 0);
            } catch (Throwable t) {
                MineAgent.LOGGER.error("Agent invocation failed, disabling agent", t);
                runner = null;
                return space.controlnet.mineagent.core.agent.AgentLoopResult.withError("Agent invocation failed: " + t.getMessage(), 0);
            }
        }
        ...
        void setRateLimitCooldown(long cooldownMs) {
            ...
            try {
                ...
                MineAgent.LOGGER.warn("Failed to set rate limit cooldown", t);
            }
        }
        ...
        void setTimeoutMs(long timeoutMs) {
            ...
            try {
                ...
                MineAgent.LOGGER.warn("Failed to set timeout", t);
            }
        }
        ...
        void setMaxToolCalls(int maxToolCalls) {
            ...
            try {
                ...
                MineAgent.LOGGER.warn("Failed to set max tool calls", t);
            }
        }
        ...
        void setMaxIterations(int maxIterations) {
            ...
            try {
                ...
                MineAgent.LOGGER.warn("Failed to set max iterations", t);
            }
        }
        ...
        void setLogResponses(boolean logResponses) {
            ...
            try {
                ...
                MineAgent.LOGGER.warn("Failed to set log responses flag", t);
            }
        }
        ...
        void setMaxRetries(int maxRetries) {
            ...
            try {
                ...
                MineAgent.LOGGER.warn("Failed to set max retries", t);
            }
        }
        ...
        private Object ensureRunner() {
            ...
            initAttempted = true;
            try {
                Class<?> clazz = Class.forName("space.controlnet.mineagent.common.agent.AgentRunner");
                runner = clazz.getConstructor().newInstance();
                return runner;
            } catch (Throwable t) {
                MineAgent.LOGGER.warn("Agent runtime unavailable, continuing without LLM support", t);
                runner = null;
                return null;
            }
        }
    ...
    private static void handleApprovalDecision(ServerPlayer player, C2SApprovalDecisionPacket packet) {
        ...
        Optional<space.controlnet.mineagent.core.terminal.TerminalContext> context = binding.isPresent()
                ? TerminalContextRegistry.fromPlayerAtBinding(player, binding.get())
                : Optional.empty();
        ...
        ToolOutcome outcome = ToolRegistry.executeTool(context, proposal.toolCall(), true);
