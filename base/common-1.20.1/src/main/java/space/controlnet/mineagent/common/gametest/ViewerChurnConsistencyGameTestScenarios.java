...
public final class ViewerChurnConsistencyGameTestScenarios {
    ...
    private ViewerChurnConsistencyGameTestScenarios() {
    }

    public static void multiViewerSnapshotConsistencyUnderChurn(GameTestHelper helper, GameTestPlayerFactory playerFactory) {
        ...
        ServerPlayer owner = playerFactory.create(helper, OWNER_ID, OWNER_NAME);
        ServerPlayer viewerA = playerFactory.create(helper, VIEWER_A_ID, VIEWER_A_NAME);
        ServerPlayer viewerB = playerFactory.create(helper, VIEWER_B_ID, VIEWER_B_NAME);
        owner.setPos(0.5D, 2.0D, 0.5D);
        viewerA.setPos(1.5D, 2.0D, 0.5D);
        viewerB.setPos(2.5D, 2.0D, 0.5D);

        AgentGameTestSupport.ensurePlayerResolvableByChatNetwork("task8/setup/player-lookup/owner", server, owner);
        AgentGameTestSupport.ensurePlayerResolvableByChatNetwork("task8/setup/player-lookup/viewer-a", server, viewerA);
        AgentGameTestSupport.ensurePlayerResolvableByChatNetwork("task8/setup/player-lookup/viewer-b", server, viewerB);
        ...
        try {
            ...
            AgentGameTestSupport.invokeHandleOpenSession(viewerA, sessionId);
            ...
            AgentGameTestSupport.invokeHandleOpenSession(viewerB, sessionId);
            requireSessionSubscriptions(
    ...
    private static void recordBroadcastStep(
        ...
        Set<UUID> recipientsBefore = AgentGameTestSupport.viewersForSession(sessionId);
        AgentGameTestSupport.requireEquals(assertionPrefix + "/recipients-before", expectedRecipients, recipientsBefore);

        MineAgentNetwork.SESSIONS.appendMessage(
                sessionId,
                new ChatMessage(ChatRole.USER, "task8-seq-" + sequence, sequence)
        );
        AgentGameTestSupport.invokeBroadcastSessionSnapshot(sessionId);

        Set<UUID> recipientsAfter = AgentGameTestSupport.viewersForSession(sessionId);
        AgentGameTestSupport.requireEquals(assertionPrefix + "/recipients-after", expectedRecipients, recipientsAfter);
    ...
    private static void requireSessionSubscriptions(String assertionName, UUID sessionId, Set<UUID> expectedViewers) {
        Set<UUID> viewers = AgentGameTestSupport.viewersForSession(sessionId);
        AgentGameTestSupport.requireEquals(assertionName + "/viewers-for-session", expectedViewers, viewers);

        Map<UUID, UUID> byViewer = AgentGameTestSupport.sessionByViewer();
        for (UUID viewerId : expectedViewers) {
            AgentGameTestSupport.requireEquals(assertionName + "/viewer-mapping/" + viewerId, sessionId, byViewer.get(viewerId));
        }
    }
    ...
    private static void requireMonotonicAndUnique(String assertionName, List<Integer> sequence) {
        ...
        AgentGameTestSupport.requireEquals(assertionName + "/unique-size", sequence.size(), unique.size());
        for (int index = 1; index < sequence.size(); index++) {
            int previous = sequence.get(index - 1);
            int current = sequence.get(index);
            AgentGameTestSupport.requireTrue(assertionName + "/strictly-increasing/" + index, current > previous);
        }
    }
}
