...
public final class AgentLoop {
    ...
    private final ConcurrentHashMap<UUID, space.controlnet.mineagent.core.proposal.Proposal> activeProposals;
    ...
    public AgentLoop(AuditLogger auditLogger, Logger logWarning) {
        ...
        this.llmExecutor = Executors.newFixedThreadPool(2, r -> {
            Thread t = new Thread(r, "mineagent-agent-llm");
            t.setDaemon(true);
            return t;
        });
    ...
    public AgentLoopResult runLoop(AgentPlayerContext player, UUID sessionId, TerminalBinding binding,
        ...
        try {
            ...
            if (hasProposal) {
                space.controlnet.mineagent.core.proposal.Proposal proposal = activeProposals.remove(sessionId);
                if (proposal != null) {
