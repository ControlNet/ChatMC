...
## 16) Test Strategy Notes (added on 2026-02-21)

### 16.1 Current state and gaps
- Current automated coverage is primarily JUnit-based (`base/core`, `base/common-1.20.1`, `ext-ae/common-1.20.1`).
- Some regressions in `base/common-1.20.1` verify source contracts via string assertions (useful for guardrails, but not sufficient for runtime correctness).
- No Forge/Fabric GameTest harness is configured yet in current Gradle/run settings.

### 16.2 Testing pyramid for ChatMC
1. **Unit JUnit (core-first):**
   - Pure logic and deterministic contracts in `base/core` and `ext-ae/core`.
   - State machine legality, parsing/boundaries, routing/policy rules.
2. **Integration JUnit (common behavior):**
   - `base/common-1.20.1` and `ext-ae/common-1.20.1` behavior tests without full in-game world orchestration.
   - NBT/serialization, packet codec contracts, boundary/error mapping.
3. **GameTest (runtime invariants):**
   - Only for behavior that requires real server/world lifecycle, scheduler, chunk state, and mod runtime integration.

### 16.3 Scenarios that must be validated in GameTest
- **Proposal binding integrity:** approval executes only against the original bound terminal (`dimension + pos + side`), with deterministic failure when terminal is removed/unavailable.
- **Indexing gate and recovery:** `INDEXING` gating under async rebuild + datapack reload transitions back to actionable state.
- **Thread confinement:** tool execution and world mutations stay on server-safe thread boundaries.
- **Multi-viewer synchronization:** session snapshots remain consistent under open/close churn and concurrent interactions.
- **Persistence realism:** `SavedData` write/reload behavior and transient-state normalization across server lifecycle.
- **End-to-end tool-args boundary:** enforce unified `65536` limit across parse/network/persistence paths.
- **AE2 runtime lifecycle:** request craft → status → cancel under real AE2 network/crafting state.

### 16.4 Cross-loader sharing strategy
- Keep **scenario logic shared** (common test intent and assertions), and keep loader-specific code as thin wrappers only.
- Use **Forge/Fabric-specific registration/entrypoint glue** for GameTest discovery and execution.
- Recommended rollout order: **Forge first** (stabilize scenarios and infra), then mirror on Fabric.

### 16.5 CI layering recommendation
- **PR gate:** all JUnit (unit + integration).
- **Main-branch gate (or scheduled gate):** Forge base GameTests for high-risk runtime scenarios.
- **Nightly/periodic:** Fabric parity GameTests and AE2-heavy GameTests.

### 16.6 First GameTest batch (high ROI)
1. Approval decision when bound terminal disappears mid-flight.
2. Indexing-not-ready gate with post-reload recovery.
3. Multi-viewer snapshot consistency under viewer churn.
4. Server-thread confinement for tool execution path.
5. Boundary tests for tool-args length (`65536` pass, overflow reject with stable signals).

### 16.7 Practical run/config notes (implementation reminder)
- **Forge:** use `runGameTestServer`; include required namespace flags for test discovery in multi-module setups.
- **Fabric:** use `fabric-gametest` entrypoint and `-Dfabric-api.gametest` system property; emit JUnit-style report file for CI collection.
