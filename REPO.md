...
- After capturing a screenshot, `AiTerminalUiAutomation` schedules a `Runtime.halt(0)` failsafe 10 seconds after `minecraft.stop()`. This prevents Forge's non-daemon threads from keeping the JVM alive indefinitely; on Fabric (which exits cleanly) the daemon halt thread is never reached.
- Both capture scripts (`capture_status_ui.py`, `capture_ui_preview.py`) now use `Popen`-based output monitoring instead of blocking `subprocess.run`. The script watches for the `[mineagent-ui-capture] saved` marker and then kills the process group immediately, avoiding unnecessary waits even when the JVM lingers.
...
- **Root cause:** No text shadow was enabled on the dark UI background, making the bitmap font appear thin and hard to read.
