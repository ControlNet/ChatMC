# Decisions — mineagent-test-pyramid-gametest-adoption
...
- Keep async/indexing/thread orchestration helpers loader-agnostic and isolated under `space.controlnet.mineagent.common.testing` so downstream tests can reuse them across Forge/Fabric wrappers.
...
- Register `ProposalBindingUnavailableGameTest` explicitly in Forge `MineAgentGameTestBootstrap` so Task 6 stays in loader-thin registration and scenario logic remains in a dedicated class.
...
- Register `ServerThreadConfinementGameTest` in `MineAgentGameTestBootstrap` to keep loader wiring explicit and scenario logic isolated in a dedicated Forge GameTest holder.
...
- Register AE GameTests through a dedicated Forge bootstrap class (`MineAgentAeGameTestBootstrap`) and an `ext-ae` `gameTestServer` Loom run namespace (`mineagentae`) so loader wiring remains explicit and isolated from base Forge scenarios.
